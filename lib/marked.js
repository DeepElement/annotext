 ...
 * Mad Props: https://github.com/chjj/marked
 */

var annotext = require('../bin/annotext');
/**
 * Block-Level Grammar
 */
  ...
  var att = function(cap) {
    if (this.annotextDoc != null && cap.length > 0) {
      ...
      revisions.forEach(function(revision) {
        if (userList.indexOf(revision.user) == -1)
          userList.push(revision.user);
        if (revisionList.indexOf(revision.revision) == -1)
          revisionList.push(revision.revision);
        if (createdList.indexOf(revision.created) == -1)
          createdList.push(revision.created);
      });
      var attribution = 'data-annotext-users=\"' + arrayToCommaDelimited(userList) + "\" data-annotext-revisions=\"" + arrayToCommaDelimited(revisionList) + "\" data-annotext-edits=\"" + arrayToCommaDelimited(createdList) + "\""
        + ' data-annotext-type=\"inline\"';
      return " " + attribution;
    } else
    return "";
  }
 ...
 */
 Parser.prototype.tok = function() {
  ...
  if (hasAttribution) {
    ...
    var revisionList = [];
    var createdList = [];
    this.token.revisions.forEach(function(revision) {
      if (userList.indexOf(revision.user) == -1)
        userList.push(revision.user);
      if (revisionList.indexOf(revision.revision) == -1)
        revisionList.push(revision.revision);
    ...
    attribution = 'data-annotext-users=\"' + arrayToCommaDelimited(userList) + "\" data-annotext-revisions=\"" + arrayToCommaDelimited(revisionList) + "\" data-annotext-edits=\"" + arrayToCommaDelimited(createdList) + "\""
     + ' data-annotext-type=\"block\"';
  }
  var att = function() {
    if (hasAttribution) {
      return " " + attribution;
